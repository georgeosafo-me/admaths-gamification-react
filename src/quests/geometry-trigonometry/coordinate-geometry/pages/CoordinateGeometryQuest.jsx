...
import { Trophy, Calculator, HelpCircle, Target, Sparkles, X, Loader2, RefreshCcw, Layers } from 'lucide-react';
import { TEMPLATES, INITIAL_DATA } from '../data/questions';
...
const CoordinateGeometryQuest = ({ onComplete }) => {
  ...
  const generateNewPuzzle = async () => {
    ...
    const prompt = `
      You are a puzzle generator. Create a JSON object for a Coordinate Geometry crossword with THREE distinct difficulty levels.
      Each level must be a valid, interlocking puzzle that fits the EXACT same grid structure below.

      GRID STRUCTURE:
      ${nextTemplate.aiInstructions}

      TOPIC: High School Coordinate Geometry (Distance, Midpoint, Slope, Lines).

      REQUIREMENTS:
      1. "easy": DoK Level 1. Direct application of formulas (e.g., "Find distance between (0,0) and (3,4)"). Simple integer answers.
      2. "medium": DoK Level 2. Multi-step or working backwards (e.g., "Midpoint is (2,3), one end is (0,0), find x of other").
      3. "hard": DoK Level 3. Strategic thinking, combining concepts (e.g., "Area of triangle formed by lines...").
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 md:p-8 font-sans relative flex flex-col">
      ...
      {aiModalOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-slate-800 border border-indigo-500/50 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-6 min-h-[150px] flex items-center justify-center">
              {aiLoading ? (
                ...
                <div className="text-slate-200 leading-relaxed text-lg math-content" dangerouslySetInnerHTML={{ __html: aiResponse }} />
              )}
            </div>
            {!aiLoading && (
              <div className="p-4 bg-slate-900/30 border-t border-slate-700 flex justify-end">
                <button onClick={() => setAiModalOpen(false)} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-medium transition-colors">
                  Got it!
                </button>
              </div>
            )}
            ...
            <header className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-emerald-400 flex items-center gap-2">
                  <Target className="w-6 h-6" /> Coordinate Mission
                </h1>
      ...
      {/* FOOTER BRANDING */}
      <footer className="w-full mt-12 py-6 border-t border-slate-800/50">
        <div className="max-w-5xl mx-auto px-4 flex flex-col md:flex-row items-center justify-center gap-2 text-center">
            ...
            <p className="text-slate-500 text-sm font-medium">
               CEO <span className="text-blue-400">Stemfinte Education Network</span>
            </p>
        </div>
      </footer>
    </div>
